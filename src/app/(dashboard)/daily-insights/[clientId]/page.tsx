...
export default function ClientInsightsPage() {
  ...
  const handleAccept = (article: DailyInsight) => {
    // Route to add web story page with URL pre-filled
    const params = new URLSearchParams({
      insightUrl: article.url,
      insightId: article.id,
    })
    router.push(`/media/web/add?${params.toString()}`)
  }
  ...
  const handleView = (article: DailyInsight) => {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-1 justify-end">
            <Button variant="ghost" size="sm" onClick={() => handleView(article)} title="View article">
