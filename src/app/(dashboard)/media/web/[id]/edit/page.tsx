...
import { Camera, ChevronRight, ChevronLeft, Loader2, Globe, Calendar, User, FileText, Image as ImageIcon, Trash2, Wand2, ArrowLeft } from 'lucide-react'
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (story.images?.length) {
          setExtractedImages(story.images.map((img: any) => img.url))
        }
      } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/media/web')} variant="outline">Back to Stories</Button>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white py-6 px-6 shadow-lg">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-orange-100 text-sm mt-1">Update article details and content</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        {formData.sourceUrl && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <div>
                <h2 className="font-semibold text-gray-800">Source URL</h2>
                <a href={formData.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-orange-500 hover:underline">{formData.sourceUrl}</a>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <textarea id="summary" className="w-full min-h-[120px] rounded-lg border border-gray-200 p-4 resize-y focus:ring-2 focus:ring-orange-500 focus:border-transparent" value={formData.summary} onChange={(e) => setFormData({ ...formData, summary: e.target.value })} placeholder="Click 'Analyze with AI' to generate a summary and sentiment analysis, or write your own..." />
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select id="publication" className="w-full h-11 rounded-lg border border-gray-200 px-3 bg-white focus:ring-2 focus:ring-orange-500" value={formData.publicationId} onChange={(e) => setFormData({ ...formData, publicationId: e.target.value })}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {extractedImages.map((imageUrl, index) => (
                <div key={index} className="relative group">
                  ...
                  <button type="button" onClick={() => handleRemoveImage(index)} className="absolute top-2 right-2 p-1.5 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                    <Trash2 className="h-3 w-3" />
                  </button>
                </div>
              ))}
              <div className="aspect-video border-2 border-dashed border-gray-200 rounded-lg flex flex-col items-center justify-center hover:border-orange-300 cursor-pointer">
                <Camera className="h-8 w-8 text-gray-400 mb-2" /><p className="text-sm text-gray-500">Add more</p>
              </div>
            </div>
          ) : (
            <div className="border-2 border-dashed border-gray-200 rounded-lg p-8 text-center hover:border-orange-300 cursor-pointer">
        ...
        <div className="flex justify-end gap-4 pt-4">
          ...
          <Button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white px-8" disabled={isSubmitting}>
            {isSubmitting ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Updating...</>) : 'Update Article'}
          </Button>
        </div>
      </form>
    </div>
