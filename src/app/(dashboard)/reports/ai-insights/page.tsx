...
export default function AIInsightsPage() {
  ...
  const handleExport = () => {
    ...
    const blob = new Blob([`<!DOCTYPE html><html><head><meta charset="utf-8"><title>AI Insights Report</title></head><body style="font-family:sans-serif;max-width:800px;margin:0 auto;">${html}</body></html>`], { type: 'text/html' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `AI-Insights-${report?.clientName || 'Report'}-${new Date().toISOString().split('T')[0]}.html`
    a.click()
  ...
  const handleSendToClient = async () => {
    ...
    try {
      const res = await fetch('/api/reports/send-to-client', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId: selectedClient, reportType: 'monthly' }),
      })
