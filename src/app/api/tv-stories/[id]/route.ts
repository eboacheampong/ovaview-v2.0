...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const story = await prisma.tVStory.findUnique({
      where: { id },
    ...
    console.error('Error fetching TV story:', error)
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.tVStorySubIndustry.deleteMany({ where: { tvStoryId: id } })

    const story = await prisma.tVStory.update({
      where: { id },
      data: {
        title, content, summary, presenters, keywords, videoUrl, videoTitle,
        date: new Date(date),
    ...
    console.error('Error updating TV story:', error)
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params
    await prisma.tVStory.delete({ where: { id } })
    return NextResponse.json({ message: 'Story deleted successfully' })
  } catch (error) {
    console.error('Error deleting TV story:', error)
