...
export async function PUT(
  ...
  try {
    ...
    // Delete existing sub-industries and images
    await prisma.webStorySubIndustry.deleteMany({ where: { webStoryId: id } })
    await prisma.webStoryImage.deleteMany({ where: { webStoryId: id } })
    ...
    const story = await prisma.webStory.update({
      ...
      data: {
        ...
        subIndustries: subIndustryIds?.length
          ? {
              create: subIndustryIds.map((subId: string) => ({
                subIndustryId: subId,
              })),
            }
          : undefined,
