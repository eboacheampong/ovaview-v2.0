...
// X (formerly Twitter) — current logo
...
// Helper to get the right icon component by platform name
export function SocialPlatformIcon({ platform, size = 16, className }: { platform: string; size?: number; className?: string }) {
  ...
  switch (p) {
    ...
    case 'REDDIT': return <RedditIcon size={size} className={className} />
    default: return <span className={className}>{platform.charAt(0)}</span>
  }
}
